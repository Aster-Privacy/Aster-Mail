...
export function use_protected_folder(
  ...
  const unlock_folder = useCallback(
    async (password: string): Promise<boolean> => {
      if (!folder?.is_password_protected || !folder?.password_set) {
        set_error("Folder does not have password protection enabled");
      ...
      try {
        ...
        if (!fetch_response.ok) {
          set_error("Failed to unlock folder");
    ...
    [folder_id, folder, reset_timeout],
  );

  const set_password = useCallback(
    async (password: string): Promise<boolean> => {
      if (folder?.password_set) {
        set_error("Password is already set. Use change_password instead.");
  ...
  const change_password_fn = useCallback(
    async (
      ...
      if (!unlocked) {
        set_error("Folder must be unlocked first");
  ...
  const remove_password_fn = useCallback(
    async (password: string): Promise<boolean> => {
      ...
      if (folder?.folder_type === "default_secure") {
        set_error("Cannot remove password from Vault folder");
