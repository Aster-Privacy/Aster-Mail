...
vi.mock("@/services/crypto/memory_key_store", () => ({
  ...
  on_keys_ready: (callback: () => void) => {
    mocks.keys_ready_callbacks.push(callback);
    callback();
...
function render_hook(): { states: number[]; root: Root } {
  const states: number[] = [];

  function Harness() {
    const r = use_category_inbox("primary", 0, true);

    states.push(r.state.emails.length);
...
describe("use_category_inbox keys-ready replay", () => {
  beforeEach(() => {
    (
      globalThis as unknown as { IS_REACT_ACT_ENVIRONMENT: boolean }
    ).IS_REACT_ACT_ENVIRONMENT = true;
    mocks.fetch_mail_by_ids_reconciled.mockClear();
    mocks.keys_ready_callbacks.length = 0;
  });
  ...
  it("keeps loaded rows when keys-ready fires again for the same account", async () => {
    const { states, root } = render_hook();

    await flush();

    expect(states.at(-1)).toBeGreaterThan(0);

    const fetches_before = mocks.fetch_mail_by_ids_reconciled.mock.calls.length;

    await act(async () => {
      mocks.keys_ready_callbacks.forEach((callback) => callback());
    });
    await flush();
