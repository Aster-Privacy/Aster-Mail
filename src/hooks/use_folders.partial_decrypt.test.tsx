...
vi.mock("@/services/crypto/legacy_keks", () => ({
  decrypt_aes_gcm_with_fallback: vi.fn(
    async (_key: unknown, data: Uint8Array) => {
      if (data[0] === 0) throw new Error("bad key");
...
describe("partial folder decryption failure", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    clear_folders_cache();
  });
  ...
  it("keeps the undecryptable folder visible with a placeholder name", async () => {
    ...
    expect(latest!.state.folders).toHaveLength(2);
    expect(latest!.state.folders.find((f) => f.id === "f1")?.name).toBe("Work");
    expect(latest!.state.folders.find((f) => f.id === "f2")?.name).toBe(
