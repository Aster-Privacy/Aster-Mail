...
describe("category_index removal tombstones", () => {
  beforeEach(async () => {
    install_fake_idb();
    idb_data.clear();
  ...
  it("keeps a removed id out when a stale sync still returns it", async () => {
    ...
    remove_ids(["keep1"]);
    expect(get_page_ids("primary", 0, 50)).toEqual(["keep2"]);
  ...
  it("lets a reindexed id back in after an undo", async () => {
    await init_category_index();
    await flush();
