...
export function extract_metadata_from_server(
  ...
  return {
    ...
    is_trashed: server_data.is_trashed ?? decrypted.is_trashed,
    is_archived: server_data.is_archived ?? decrypted.is_archived,
    is_spam: server_data.is_spam ?? decrypted.is_spam,
    has_attachments:
    ...
    size_bytes: Math.max(
      decrypted.size_bytes ?? 0,
      server_data.size_bytes ?? 0,
    ),
