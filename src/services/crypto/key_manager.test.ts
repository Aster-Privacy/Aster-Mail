...
import {
  ...
  import_ecdh_public_key,
  import_ecdh_private_key,
  derive_shared_secret_as_crypto_key,
  derive_shared_secret_bits,
...
describe("ECDH Key Operations", () => {
  describe("generate_ecdh_keypair", () => {
    ...
    it("should generate unique keypairs", async () => {
      const keypair1 = await generate_ecdh_keypair();
      const keypair2 = await generate_ecdh_keypair();

      expect(keypair1.public_key_raw).not.toEqual(keypair2.public_key_raw);
    });
  });

  describe("import_ecdh_public_key", () => {
    it("should import raw public key", async () => {
      const keypair = await generate_ecdh_keypair();
      const imported = await import_ecdh_public_key(keypair.public_key_raw);
  ...
  describe("derive_shared_secret_bits", () => {
    it("should derive shared secret between two parties", async () => {
      const alice = await generate_ecdh_keypair();
      const bob = await generate_ecdh_keypair();

      const alice_public = await import_ecdh_public_key(alice.public_key_raw);
      const bob_public = await import_ecdh_public_key(bob.public_key_raw);

      const alice_secret = await derive_shared_secret_bits(
        alice.private_key,
        bob_public
      );
      const bob_secret = await derive_shared_secret_bits(
        bob.private_key,
        alice_public
      );
    ...
    it("should produce 32-byte shared secret", async () => {
      ...
      const secret = await derive_shared_secret_bits(
        alice.private_key,
        bob_public
      );
  ...
  describe("derive_shared_secret_as_crypto_key", () => {
    it("should derive usable AES key from ECDH", async () => {
      const alice = await generate_ecdh_keypair();
      const bob = await generate_ecdh_keypair();
      const bob_public = await import_ecdh_public_key(bob.public_key_raw);

      const aes_key = await derive_shared_secret_as_crypto_key(
        alice.private_key,
        bob_public
      );
...
describe("Encryption/Decryption with CryptoKey", () => {
  ...
  it("should fail decryption with wrong key", async () => {
    const key1 = await import_aes_key(crypto.getRandomValues(new Uint8Array(32)));
    const key2 = await import_aes_key(crypto.getRandomValues(new Uint8Array(32)));

    const plaintext = new TextEncoder().encode("Secret");
    const { ciphertext, nonce } = await encrypt_with_crypto_key(plaintext, key1);
  ...
  it("should fail decryption with wrong nonce", async () => {
    const key = await import_aes_key(crypto.getRandomValues(new Uint8Array(32)));
