...
export async function perform_x3dh_sender(
  ...
  const recipient_identity_public = await import_ecdh_public_key(
    recipient_identity_raw,
  );
  const recipient_signed_prekey_public = await import_ecdh_public_key(
  ...
  const dh2 = await derive_shared_secret_bits(
    ephemeral.private_key,
    recipient_identity_public,
  );

  const dh3 = await derive_shared_secret_bits(
    ephemeral.private_key,
    recipient_signed_prekey_public,
  );
...
export async function perform_x3dh_receiver(
  ...
  const receiver_identity_private = await import_ecdh_private_key(
    receiver_identity_jwk,
  );
  const receiver_signed_prekey_private = await import_ecdh_private_key(
