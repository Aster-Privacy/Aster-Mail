...
async function import_private_key(private_key: Uint8Array): Promise<CryptoKey> {
  const x_y = await generate_public_from_private(private_key);
  const jwk: JsonWebKey = {
    kty: "EC",
    crv: "P-256",
    d: array_to_base64(private_key)
      .replace(/\+/g, "-")
      .replace(/\//g, "_")
      .replace(/=/g, ""),
...
async function generate_public_from_private(
  ...
  jwk.d = array_to_base64(private_key)
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=/g, "");
...
export class DoubleRatchet {
  ...
  static async init_sender(
    ...
    const private_key = await import_private_key(dh_keypair.private_key);
    const public_key = await import_public_key(remote_public_key);
    const dh_output = await dh(private_key, public_key);
    ...
    const state: RatchetState = {
      dh_keypair: {
        ...
        private_key: array_to_base64(dh_keypair.private_key),
      },
  ...
  static async init_receiver(
    ...
    const state: RatchetState = {
      dh_keypair: {
        ...
        private_key: array_to_base64(own_keypair.private_key),
      },
  ...
  private async dh_ratchet(remote_public_key: Uint8Array): Promise<void> {
    ...
    const private_key = await import_private_key(
      base64_to_array(this.state.dh_keypair.private_key),
    );
    const public_key = await import_public_key(remote_public_key);
    ...
    this.state.dh_keypair = {
      ...
      private_key: array_to_base64(new_dh_keypair.private_key),
    };

    const new_private_key = await import_private_key(
      new_dh_keypair.private_key,
    );
    const new_dh_output = await dh(new_private_key, public_key);
    ...
    secure_zero_memory(chain_key);
    secure_zero_memory(new_dh_keypair.private_key);
