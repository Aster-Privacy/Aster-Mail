...
describe("pq prekey negative cache (404 flood fix)", () => {
  ...
  it("persists the negative result across a fresh module-cache load", async () => {
    get_mock.mockResolvedValue({ error: "not found", code: "NOT_FOUND" });
    await load_pq_secret(688968);
    expect(get_mock).toHaveBeenCalledTimes(1);
    // localStorage now holds the missing entry; a later lookup still skips fetch
    await load_pq_secret(688968);
    expect(get_mock).toHaveBeenCalledTimes(1);
    expect(store.get("pq_prekey_missing")).toContain("688968");
  });
