...
function read_head_unread_sibling() {
  return {
    data: {
      items: [
        {
...
describe("category_index ghost unread", () => {
  beforeEach(async () => {
    install_fake_idb();
    idb_data.clear();
    ...
    list_mail_items.mockResolvedValue(read_head_unread_sibling());
  ...
  it("marks the representative unread when a collapsed thread has an unread sibling behind a read head", async () => {
