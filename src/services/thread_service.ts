  ...
  const decrypt_promises = messages_to_decrypt.map(async (msg) => {
    ...
    if (!envelope) {
      return {
        ...
        is_read: msg.is_read ?? decrypted_metadata?.is_read ?? false,
        is_starred: msg.is_starred ?? decrypted_metadata?.is_starred ?? false,
        is_deleted: false,
  ...
  const decrypt_promises = visible_items.map(async (item) => {
    ...
    if (!envelope) {
      return {
        ...
        is_starred: item.is_starred ?? decrypted_metadata?.is_starred ?? false,
        is_deleted: false,
