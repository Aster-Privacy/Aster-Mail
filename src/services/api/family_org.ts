...
export const list_org_groups = () =>
  api_client.get<OrgGroup[]>(`${BASE}/groups`);
export const create_org_group = (data: {
  name: string;
  description?: string;
...
export const delete_org_group = (id: string) =>
  api_client.delete<unknown>(`${BASE}/groups/${id}`);
export const list_group_members = (id: string) =>
  api_client.get<OrgGroupMember[]>(`${BASE}/groups/${id}/members`);
...
export const get_activity_log = (
  page = 1,
  per_page = 50,
  event_type?: string,
) =>
  api_client.get<ActivityLogResponse>(
    `${BASE}/activity?page=${page}&per_page=${per_page}${event_type ? `&event_type=${encodeURIComponent(event_type)}` : ""}`,
  );

export const list_org_filters = () =>
  api_client.get<OrgFilter[]>(`${BASE}/filters`);
export const create_org_filter = (data: {
  name: string;
  filter_type: string;
  field: string;
  value: string;
...
export const update_org_filter = (
  id: string,
  data: { name?: string; is_enabled?: boolean; priority?: number },
) => api_client.patch<OrgFilter>(`${BASE}/filters/${id}`, data);
export const delete_org_filter = (id: string) =>
  api_client.delete<unknown>(`${BASE}/filters/${id}`);

export const get_data_retention = () =>
  api_client.get<DataRetentionPolicy>(`${BASE}/retention`);
export const update_data_retention = (policy: DataRetentionPolicy) =>
  api_client.put<DataRetentionPolicy>(`${BASE}/retention`, policy);

export const get_security_policy = () =>
  api_client.get<SecurityPolicy>(`${BASE}/security`);
export const update_security_policy = (policy: Partial<SecurityPolicy>) =>
  api_client.patch<SecurityPolicy>(`${BASE}/security`, policy);

export const list_family_domains = () =>
  api_client.get<FamilyDomain[]>(`${BASE}/domains`);
export const share_domain = (
  domain: string,
  user_id: string,
  can_create_aliases = true,
) =>
  api_client.post<unknown>(
    `${BASE}/domains/${encodeURIComponent(domain)}/share`,
    { user_id, can_create_aliases },
  );
export const revoke_domain_share = (domain: string, user_id: string) =>
  api_client.delete<unknown>(
    `${BASE}/domains/${encodeURIComponent(domain)}/share/${user_id}`,
  );

export const get_member_compliance = () =>
  api_client.get<MemberComplianceInfo[]>(`${BASE}/compliance`);

export const notify_non_compliant_2fa = () =>
  api_client.post<{ notified: number }>(
    "/payments/v1/family/org/notify-2fa",
    {},
  );
...
export const create_consent_request = (
  ...
  api_client.post<PendingConsentRequest>(`${BASE}/consent-requests`, {
    kind,
    description,
    payload,
  });
...
export const list_member_consent_requests = () =>
  api_client.get<MemberConsentRequest[]>(
    "/payments/v1/family/member/consent-requests",
  );

export const respond_consent_request = (id: string, accepted: boolean) =>
  api_client.post<void>(
    `/payments/v1/family/member/consent-requests/${id}/respond`,
    { accepted },
  );
...
export const get_member_retention_policy = () =>
  api_client.get<MemberRetentionPolicy>(
    "/payments/v1/family/member/retention-policy",
  );
