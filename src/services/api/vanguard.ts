...
export interface BreachListEntry {
  domain: string;
  name: string;
  breach_date: string;
  disclosed_at: string;
...
interface BreachListApiResponse {
  breaches: BreachListEntry[];
}

export async function get_breach_list(
  updated_since?: string,
): Promise<ApiResponse<BreachListEntry[]>> {
  const query = updated_since
    ? `?updated_since=${encodeURIComponent(updated_since)}`
    : "";

  const response = await api_client.get<BreachListApiResponse>(
    `/core/v1/security/vanguard/breach-list${query}`,
    { cache_ttl: 0 },
  );
  ...
  return { data: response.data.breaches };
}
...
export async function notify_breach_match(
  match_count: number,
): Promise<ApiResponse<BreachAlertResponse>> {
  return api_client.post<BreachAlertResponse>(
    "/core/v1/security/vanguard/breach-alert",
    { match_count },
  );
}
