...
describe("a truncated inbox walk never prunes the index", () => {
  beforeEach(() => {
    clear_category_index_memory();
    mocked_list.mockReset();
  });

  it("keeps indexed mail when the server reports more pages but sends no cursor", async () => {
    ...
    seed();
    mocked_list.mockResolvedValue({
      data: { items: [], total: 1, has_more: true, next_cursor: undefined },
    });
    await build_index({ force: true });
  ...
  it("still prunes when the server confirms the walk reached the end", async () => {
    mocked_list.mockResolvedValue({
      data: { items: [], total: 0, has_more: false },
    });
    await build_index({ force: true });
    seed();
    mocked_list.mockResolvedValue({
      data: { items: [], total: 0, has_more: false, next_cursor: undefined },
    });
    await build_index({ force: true });
