...
export function ContactList({
  ...
  const letters = useMemo(
    () => Array.from(alphabetical_index.keys()),
    [alphabetical_index],
  );
  ...
  return (
    <div className="w-full md:w-1/2 md:flex-shrink-0 md:min-w-0 md:border-e md:border-edge-primary min-h-0 flex flex-col">
      <div className="flex items-center gap-2 px-4 pt-4 pb-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              className="h-8 px-2 gap-1 text-[12px]"
              size="md"
              variant="outline"
            >
              {sort_by === "name_desc" ? (
                <BarsArrowUpIcon className="w-4 h-4 text-txt-secondary" />
              ) : (
                <BarsArrowDownIcon className="w-4 h-4 text-txt-secondary" />
              )}
              <span className="hidden lg:inline text-txt-secondary">
                {sort_label}
              </span>
            </Button>
          </DropdownMenuTrigger>
        ...
        >
          {is_compact ? (
            <Bars3Icon className="w-4 h-4 text-txt-secondary" />
          ) : (
            <Bars2Icon className="w-4 h-4 text-txt-secondary" />
          )}
        </Button>
        <Button
          aria-label={t("common.import_contacts")}
          className="h-8 w-8"
      ...
      <div className="flex-1 min-h-0 flex">
            ...
            filtered_contacts.map((contact) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "group/contact w-full flex items-center rounded-[12px] text-start transition-colors",
                    is_compact
                      ? "gap-2 px-2 py-0.5 my-0"
                      : "gap-3 px-3 py-1 my-0.5",
                    is_selected
        ...
        {!is_loading && letters.length > 1 && (
          <div className="hidden md:flex flex-col items-center justify-start gap-0.5 pe-1 py-2 overflow-y-auto">
            {letters.map((letter) => (
              <button
                key={letter}
