...
import { ProfileNotesInline } from "@/components/profile_notes_inline";
...
export function ProfileDropdown({
  ...
  const handle_contact_action = useCallback(async () => {
    ...
    try {
      if (existing_contact_id) {
        ...
        if (result.data) {
          show_toast("Removed from contacts", "success");
          set_existing_contact_id(null);
          checked_email_ref.current = null;
        ...
        if (result.data) {
          show_toast("Added to contacts", "success");
      ...
      show_toast("Failed to update contact", "error");
    } finally {
  ...
  const handle_block_sender = useCallback(async () => {
    ...
    try {
      ...
      if (result.data) {
        show_toast(`Blocked ${email}`, "success");
        set_is_open(false);
        emit_mail_changed();
      } else if (result.error) {
        show_toast(result.error, "error");
      ...
      show_toast("Failed to block sender", "error");
    } finally {
    ...
    <DropdownMenu open={is_open} onOpenChange={set_is_open}>
      ...
      >
        <div className="px-3 pt-3 pb-2">
          ...
          <button
            className="mt-2 w-full flex items-center justify-center gap-1.5 py-1.5 rounded-md text-[12px] transition-colors border"
            style={{
        ...
        >
          ...
          <span>{show_notes ? "Hide notes" : "Notes"}</span>
        </DropdownMenuItem>
        ...
        >
          ...
          <span>Messages from this sender</span>
        </DropdownMenuItem>
