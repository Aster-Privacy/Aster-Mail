      ...
      if (result.locked) {
        ...
        const msg =
          result.attempts_remaining > 0
            ? t("settings.app_lock_attempts_remaining", {
...
function SetupPinModal({
  ...
  const [chosen_mode, set_chosen_mode] = useState<"numeric" | "text">(
    "numeric",
  );
      ...
      if (next.length === chosen_digits) {
        ...
        try {
          ...
          const duress_fields = carrying_duress
            ...
            : {};
          const use_pepper =
            !carrying_duress || existing?.kdf_version === KDF_VERSION_PEPPER;
          const pepper = use_pepper
          ...
          save_app_lock_config(account_id, {
            ...
            pin_hash,
            pin_salt,
            kdf_version: pepper ? KDF_VERSION_PEPPER : undefined,
            ...duress_fields,
          });

          if (duress_collides) {
            show_toast(t("settings.duress_pin_matches_regular"), "warning");
          } else if (dropping_duress) {
            show_toast(
              t("settings.duress_pin_cleared_length_change"),
              "warning",
            );
          }
  ...
  const step_index =
    step === "choose_mode"
      ? 0
      : step === "choose_digits" || step === "set_text"
        ? 1
        : 2;
  ...
  const modal_title =
    step === "choose_mode"
      ? t("settings.app_lock_choose_mode")
      : step === "choose_digits"
        ? t("settings.app_lock_choose_digits")
        : step === "set_pin"
          ? t("settings.app_lock_set_pin")
          : step === "confirm_pin"
            ? t("settings.app_lock_confirm_pin")
            : step === "set_text"
              ? t("settings.app_lock_set_passphrase")
              : t("settings.app_lock_confirm_passphrase");

  return (
    <Modal
      close_on_overlay={is_first_step}
      is_open={is_open}
      on_close={on_close}
      size="sm"
    >
      ...
      <ModalBody>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              <div className="font-medium">
                {t("settings.app_lock_mode_numeric")}
              </div>
              <div
                className={cn(
                  "text-xs mt-0.5",
            ...
            >
              <div className="font-medium">
                {t("settings.app_lock_mode_text")}
              </div>
              <div
                className={cn(
                  "text-xs mt-0.5",
                  chosen_mode === "text" ? "text-white/70" : "text-txt-muted",
                )}
              >
        ...
        {step === "choose_digits" && (
          <div className="flex flex-col gap-2">
            {([4, 6, 8] as const).map((n) => (
              ...
              >
                {n === 4
                  ? t("settings.app_lock_digits_4")
                  : n === 6
                    ? t("settings.app_lock_digits_6")
                    : t("settings.app_lock_digits_8")}
              </button>
            ))}
          </div>
        )}
        {step === "set_pin" && (
          <div className="flex flex-col items-center gap-2 pt-1">
            <PinDots
              digits={chosen_digits}
              filled={first_pin.length}
              shake_key={0}
            />
            <PinPad
              on_backspace={handle_first_backspace}
        ...
        {step === "confirm_pin" && (
          <div className="flex flex-col items-center gap-2 pt-1">
            ...
            <PinPad
              disabled={saving}
              on_backspace={handle_confirm_backspace}
...
export function AppLockSection() {
  ...
  useEffect(() => {
    if (account_id)
      set_enabled(get_app_lock_config(account_id)?.enabled ?? false);
  }, [account_id]);
