...
export function use_security_fetchers(ctx: security_fetchers_context) {
  ...
  const fetch_login_events = useCallback(async () => {
    ...
    try {
      ...
      if (response.data) {
        ...
        set_login_events_failed(false);
      } else {
        set_login_events_failed(true);
      }
    } catch (error) {
      if (import.meta.env.DEV) console.error(error);
      set_login_events_failed(true);
  ...
  const fetch_ipfs_status = useCallback(async () => {
    try {
      ...
      if (response.data) {
        ...
        set_ipfs_status_loaded(true);
      } else {
        set_ipfs_status_loaded(false);
      }
    } catch (error) {
      if (import.meta.env.DEV) console.error(error);
      set_ipfs_status_loaded(false);
  ...
  const fetch_recovery_email_status = useCallback(async () => {
    ...
    try {
      ...
      ignore_error(
        "components/settings/hooks/use_security/fetchers:use_security_fetchers",
        caught,
      );
    }
  }, [cache]);
    ...
    try {
      ...
      if (status.password_strength_tier === null) {
        ...
        if (passphrase) {
          ...
          backfill_password_strength_tier(tier).catch((caught) =>
            ignore_error(
              "components/settings/hooks/use_security/fetchers:use_security_fetchers",
              caught,
            ),
  ...
  const hydrate_security_status = useCallback(async () => {
    const cached =
      cache.get_entry<ApiResponse<SecurityStatusResponse>>("security_status");
  ...
  const hydrate_totp_status = useCallback(async () => {
    const cached =
      cache.get_entry<ApiResponse<TotpStatusResponse>>("totp_status");
