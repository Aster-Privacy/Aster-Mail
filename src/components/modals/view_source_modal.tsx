...
function tokenize_html(source: string): Token[] {
  const tokens: Token[] = [];
  let i = 0;
  ...
  return (
    <Modal is_open={is_open} on_close={on_close} size="full">
        ...
        <div className="flex items-center justify-between w-full">
          ...
          <div className="flex items-center gap-2 mr-4">
            <button
              ...
              style={{
                backgroundColor: wrap_lines
                  ? "var(--accent-muted)"
                  : "transparent",
                color: wrap_lines ? "var(--accent)" : "var(--text-muted)",
                border: "1px solid var(--border-secondary)",
              }}
              type="button"
              onClick={() => set_wrap_lines(!wrap_lines)}
            >
              Wrap
            </button>
          </div>
      ...
      <ModalFooter>
        ...
        <Button onClick={handle_copy}>Copy source</Button>
      </ModalFooter>
    </Modal>
  );
}
