...
export function ContactDuplicatesList({
  ...
  const [duplicates, set_duplicates] = useState<
  ...
  const handle_dismiss = useCallback(async (duplicate_id: string) => {
  ...
  const handle_merge_complete = useCallback(
    (_merged_contact_id: string) => {
      if (merge_candidate) {
        set_duplicates((prev) =>
          prev.filter((d) => d.id !== merge_candidate.id),
        );
        set_total((prev) => prev - 1);
      }
