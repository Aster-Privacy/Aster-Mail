...
export function ContactPhotoUpload({
  ...
  const handle_file_select = useCallback(
    async (file: File) => {
      ...
      if (!ACCEPTED_TYPES.includes(file.type)) {
        set_error(t("common.select_valid_image"));

        return;
      }

      if (file.size > MAX_FILE_SIZE) {
        set_error(t("common.image_too_large"));
  ...
  const handle_delete = useCallback(async () => {
    ...
    try {
      ...
      set_error(err instanceof Error ? err.message : t("common.delete_failed"));
    } finally {
