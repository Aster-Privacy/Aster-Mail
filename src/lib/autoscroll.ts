...
export const is_scrollable = (node: Element, axis: "x" | "y"): boolean => {
...
export const element_target = (node: Element): ScrollTarget => ({
  scroll_by: (dx, dy) => {
    node.scrollTop += dy;
    node.scrollLeft += dx;
  },
...
export const document_target = (doc: Document): ScrollTarget | null => {
...
export const find_scrollable_target = (
  start: Element | null,
): ScrollTarget | null => {
  let node = start;
...
export const start_autoscroll_session = (
  target: ScrollTarget,
  origin_x: number,
  origin_y: number,
  inner_doc?: Document | null,
  offset_of_inner?: () => { left: number; top: number },
): boolean => {
  ...
  const move_pointer = (x: number, y: number) => {
    pointer_x = x;
    pointer_y = y;
    if (
  ...
  const track_outer = (event: MouseEvent) => {
    move_pointer(event.clientX, event.clientY);
  };

  const track_inner = (event: MouseEvent) => {
    const offset = offset_of_inner?.() ?? { left: 0, top: 0 };

    move_pointer(offset.left + event.clientX, offset.top + event.clientY);
  };
...
let active_session: { stop: () => void } | null = null;

export const is_autoscrolling = (): boolean => active_session !== null;
