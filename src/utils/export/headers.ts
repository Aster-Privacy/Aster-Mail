...
export function emit_raw_header_passthrough(
  name: string,
  value: string,
): string {
  const single_line = value
    .replace(/\r?\n[ \t]+/g, " ")
    .replace(/[\r\n]/g, " ")
    .trim();
...
export function generate_message_id(
  domain: string = "export.local.astermail",
): string {
...
export function filename_param(filename: string): string {
  if (
    is_ascii_printable(filename) &&
    !SPECIALS.test(filename) &&
    !/\s/.test(filename)
  ) {
